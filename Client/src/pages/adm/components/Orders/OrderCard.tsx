      ...
      <div className="space-y-3 mb-4">
        ...
        {order.customer?.address && (
          <div className="flex items-start space-x-2 text-sm text-gray-600">
            ...
            <div className="space-y-0.5 font-bold text-base">
              <p>
  ...
  <div className="flex items-start space-x-3 bg-gray-50 p-3 rounded-lg border border-gray-200">
    <div className="flex flex-col">
      <p className="text-sm font-medium text-gray-800">
        Método de Pagamento: <span className="font-semibold capitalize">{order.paymentMethod}</span>
      </p>
      {order.needsChange && order.changeAmount && (
        <p className="text-sm text-gray-500 mt-1">
          Troco para: <span className="font-medium">{formatCurrency(Number(order.changeAmount))}</span>
        </p>
      )}
    </div>
  </div>
            ...
            return (
              ...
              >
                <span className="text-gray-600">
                  {item.quantity}x {item.product.name} ({item.product.weight} Kg){" "}
                  <br />
                ...
                <span className="font-medium text-gray-900">
                  {formatCurrency(precoUnitario * item.quantity)}
                </span>
              </div>
            );
          })
